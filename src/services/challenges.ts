...
export const updateChallengeProgress = async (
  ...
  if (snapshot.empty) {
    ...
    await updateDoc(progressDoc.ref, {
      ...
      completed,
      completedAt: completed && !wasCompleted ? new Date().toISOString() : progressDoc.data().completedAt,
    });
...
/**
 * Increment challenge progress (simpler API for common use cases)
 */
...
export const WEEKLY_CHALLENGES_TEMPLATE: Omit<Challenge, 'id' | 'startDate' | 'endDate'>[] = [
  {
    ...
    description: 'Win 15 rounds this week',
    icon: '👑',
    requirement: 15,
