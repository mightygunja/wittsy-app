...
/**
 * Get user's active RANKED game room (if any)
 * Returns the ranked room where the user is currently a player
 * Casual games are ignored - users can be in casual and ranked simultaneously
 */
export const getUserActiveRoom = async (userId: string): Promise<Room | null> => {
  try {
    ...
    // Find the first ranked room where user is a player
    for (const doc of snapshot.docs) {
      const roomData = doc.data();
      const players = roomData.players || [];
      if (players.some((p: Player) => p.userId === userId)) {
    ...
    console.error('Error getting user active ranked room:', error);
    return null;
  }
};
 ...
 * - Immediately deletes the room and kicks all players
 * - Cleans up realtime database state
 */
...
/**
 * Leave a room
 * - Fully removes the player from the room
 * - Deletes the room if no players remain
 * - Ends active games if fewer than 3 players remain
 */
 ...
 * Ranked games cannot have their settings changed
 */
export const updateRoomSettings = async (
  ...
  // Prevent settings changes for ranked games
  if (roomData.isRanked) {
    console.log('⚠️ Cannot update settings for ranked game');
    throw new Error('Cannot modify settings for ranked games');
  }
