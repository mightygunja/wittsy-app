...
/**
 * Grant event completion rewards to participant
 */
export const grantEventRewards = async (
  eventId: string,
  userId: string,
  placement: number
): Promise<void> => {
  ...
  const prize = event.prizes.find(p => {
    if (p.positionRange) {
      // Handle ranges like "Top 10", "5th-8th", "All Participants"
      if (p.positionRange.toLowerCase().includes('all')) return true;
      if (p.positionRange.toLowerCase().includes('top')) {
        const topN = parseInt(p.positionRange.match(/\d+/)?.[0] || '0');
        return placement <= topN;
      }
      // Handle ranges like "5th-8th"
      const rangeMatch = p.positionRange.match(/(\d+).*?(\d+)/);
      if (rangeMatch) {
        const [, start, end] = rangeMatch;
        return placement >= parseInt(start) && placement <= parseInt(end);
      }
    }
    return p.position === placement;
  });

  if (!prize) {
    console.log(`No prize found for placement ${placement} in event ${event.name}`);
    return;
  }
  ...
  try {
    // Grant coins
    if (prize.coins) {
      await rewards.grantCoins(userId, prize.coins, `event_${eventId}_placement_${placement}`);
    ...
    if (prize.title) {
      ...
      if (userDoc.exists()) {
        ...
        if (!unlockedTitles.includes(prize.title)) {
          await updateDoc(userRef, {
            unlockedTitles: [...unlockedTitles, prize.title],
          });
          console.log(`✅ Unlocked title "${prize.title}" for event completion`);
        }
      }
    }
    ...
    if (prize.badge) {
      ...
      if (userDoc.exists()) {
        ...
        if (!badges.includes(prize.badge)) {
          await updateDoc(userRef, {
            badges: [...badges, prize.badge],
          });
          console.log(`✅ Unlocked badge "${prize.badge}" for event completion`);
        }
      }
    }
    ...
    if (prize.items && prize.items.length > 0) {
      for (const itemId of prize.items) {
        await avatarService.unlockItem(userId, itemId, 'event');
        console.log(`✅ Unlocked avatar item ${itemId} for event completion`);
      }
    }

    // Create notification
    await createNotification(userId, 'event_reward', {
      title: `🏆 Event Rewards!`,
      message: `You placed ${getOrdinal(placement)} in ${event.name}! Check your rewards.`,
      eventId,
      placement,
      prize,
    });
    ...
    // Track analytics
    analytics.logEvent('event_reward_granted', {
      user_id: userId,
      event_id: eventId,
      ...
      coins: prize.coins || 0,
      xp: prize.xp || 0,
      has_title: !!prize.title,
      has_badge: !!prize.badge,
      has_items: !!(prize.items && prize.items.length > 0),
    });
  } catch (error) {
...
/**
 * Get user's events (registered and participated)
 */
export const getUserEvents = async (userId: string): Promise<Event[]> => {
  ...
  for (const event of allEvents) {
    const isRegistered = await isUserRegistered(event.id, userId);
    if (isRegistered) {
